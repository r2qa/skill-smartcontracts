...
> Walk every applicable item during a review. Grouped by contract type. For each: the grep-able pattern, why it's risky, and the concrete check to perform. 177 function patterns across 21 contract-type categories. Companion: [vulnerable-chains.md](vulnerable-chains.md).
...
### `USDT-TRON admin powers: destroyBlackFunds / deprecate / issue-redeem (Tether TetherToken)`
- **Risk:** TRON's dominant stablecoin (Tether `TetherToken`) is NOT a plain ERC-20 — the owner can `addBlackList` an address AND then `destroyBlackFunds(addr)` to **seize/burn that address's balance outright** (not just freeze), `deprecate(newAddr)` to **redirect every call to a new contract** (a live upgrade of an "immutable" token), plus `issue`/`redeem` supply control and `setParams` fees. These are distinct from a generic blacklist/pausable: `destroyBlackFunds` is confiscation, `deprecate` is a full redirect. Any protocol integrating USDT inherits this admin surface (funds it holds can be frozen/seized; the token can be redirected out from under it).
- **Verify:** For a USDT-integrating protocol, treat the token owner as an external privileged actor (Conditional/Centralization) — model `addBlackList`+`destroyBlackFunds` on protocol-held balances and a `deprecate` redirect; confirm the protocol degrades safely (no invariant assumes USDT is seizure-proof or its address immutable). Rate as a trust-assumption, not a code bug in the integrator, but STATE it.
...
- **Risk:** A vault whose underlying is native TRX receives value in SUN (1e6), not wei (1e18) — 18-dec share math over a 6-dec asset changes the inflation-attack economics and can mis-scale conversions by 1e12; TRON **USDT is 6 decimals but USDD is 18** (verified on-chain) — do NOT assume both are 6; a hardcoded offset misprices shares by up to 1e12x.
- **Verify:** Confirm the offset and conversion scaling match the ACTUAL asset decimals (TRX-SUN = 6, USDT = 6, **USDD = 18**) — always read `decimals()` on-chain, never assume; keep native-TRX (SUN) math distinct from TRC-20; size the virtual-share buffer in the asset's real decimals.
...
- **Verify:** Verify funding/interest integrate over timestamp not block count (or re-derive per-block rates for ~3s blocks); verify collateral decimals (TRX-SUN = 6, USDT = 6, **USDD = 18** — read `decimals()` on-chain, don't assume) match the margin math; keep native-TRX and TRC-20 collateral scaling distinct.
...
- **Risk:** Staking/resource ops carry TIME economics an EVM checklist ignores. `UnfreezeBalanceV2` enforces an **unbonding delay before `WithdrawExpireUnfreeze`** — but that delay is a **governance chain parameter** (`getUnfreezeDelayDays` / `getchainparameters`), ~14 days on mainnet, **1 day on Nile, and adjustable** — NOT a hardcoded constant. `DelegateResource` locking is **OPTIONAL**: a `lock` flag with a configurable `lock_period` (default 3 days / 86400 blocks when `lock=true`; TIP-542 makes it adjustable up to ~30 days via network param #78); **without `lock=true`, delegated resources can be un-delegated IMMEDIATELY** — so un-locked energy rental is a **mid-execution energy-starvation DoS surface** (the provider yanks the delegation), NOT a guaranteed window. A rental/withdrawal-queue/liquid-staking contract that hardcodes the wrong delay, or assumes delegation is always/never locked, mis-accounts → insolvency, stuck funds, or serviceability DoS.
...
### `SR voting-reward accounting (sTRX / liquid-staking-TRX): VoteWitness + WithdrawBalance reward distribution`
- **Risk:** On TRON, staked TRX votes for Super Representatives and earns **voting rewards** (claimed via `WithdrawBalance`) at an SR-set, **changeable** commission/rate — a reward source EVM checklists don't model. A liquid-staking wrapper (sTRX-class) distributing these rewards has a correctness surface: if the share/exchange rate is bumped from an *assumed* APY rather than the **actually-claimed** reward, or updated AFTER a mint/burn, a just-deposited attacker captures rewards accrued before their deposit (or a just-before-claim deposit dilutes holders); **unclaimed-but-pending** rewards not counted in the rate enable a first-depositor-style inflation of the staking rate; SR commission changes / vote re-allocation silently mis-price it.
- **Verify:** Confirm the share/exchange rate is updated from the **real withdrawn reward amount** (read it), and BEFORE any deposit/withdraw touches supply; confirm a deposit can't capture pre-deposit rewards (claim-before-mint or per-share checkpoint); confirm pending-unclaimed rewards are included in the rate; confirm SR-commission / re-vote changes are reflected; confirm the unbonding + reward-claim timing can't be gamed for a same-epoch in→out profit. (Actor: an unprivileged depositor gaming the rate is a Vulnerability; an admin setting the SR/commission is Centralization.)
