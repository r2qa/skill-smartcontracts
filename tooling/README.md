...
| **Semgrep** + Decurity rules | 10 | Pattern/taint static analysis | Static pass, two rulesets (gate 2): the **pinned** Decurity clone at `$AUDIT_HOME/semgrep-smart-contracts/solidity/security` (pinned by `DECURITY_REF` for reproducibility — gate 2 uses the clone, not the floating registry `p/smart-contracts`) **and** the in-repo TRON/TVM-native set `tooling/semgrep-tron/` (ships with the skill, no install — see its `README.md`). |
