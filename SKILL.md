...
1. **Get source, then build to green.** For an on-chain target, fetch verified source FIRST: `tooling/get-source.sh <address>` pulls every `.sol` + the **exact compiler settings** (`compiler.json`) + ABI from TronScan's `POST /api/solidity/contract/info` (sends `TRON-PRO-API-KEY`), and caches by `code_hash`. **Two distinct trust levels — do not conflate them (they are recorded in `compiler.json`):** (1) `explorer_source_status=2` + `node_runtime_hash_match=MATCH` means only *TronScan says verified* and *the node's runtime is self-consistent* — NOT that this `.sol` produced the deployed code; (2) **source is authoritative ONLY when `recompile_status=FULL_MATCH`** (`source_authoritative=true`) — the script recompiles the fetched source with the pinned compiler and gets `keccak256(deployedBytecode)==on-chain code_hash`. `PARTIAL` (only trailing metadata differs) is strong but not a full byte-proof; `NO_MATCH`/`SKIPPED` means treat the source as unproven and reconcile before relying on it (usually a missing TRON solc fork — run `bootstrap.sh`). `status≠2` → no source exists, drop to the **bytecode-only branch** below. Then detect toolchain (foundry.toml / hardhat.config / tronbox.js / truffle-config / vyper). Install and **pin** the exact compiler (`solc-select`/`svm`, or the TRON `tv_*` solc fork for TVM), install deps, compile. If it won't compile, fix that before reasoning — but **build fixes must never alter in-scope contract logic**; restrict them to toolchain/config/dependency/remapping fixes, and if a source edit is unavoidable, make it in a scratch copy or record it as a `*.patch` under `poc/` and disclose it, keeping the frozen baseline pristine. **Dedup guard (don't audit the same contract twice):** once source + `code_hash` are fetched, run `tooling/review-ledger.sh check <address>`. If it prints `REVIEWED-CURRENT` (same `code_hash` + impl + same skill `VERSION`) and the user did NOT explicitly ask to re-check, **STOP here and return the prior report path — do not run gates 2–9.** It auto-**proceeds** (with the reason) when the `code_hash` changed (contract redeployed/upgraded), the proxy impl changed, the skill `VERSION` bumped (new knowledge), the address is new, or `RECHECK=1` is set. A `NEW-ADDRESS-SAME-BYTECODE` result means identical code was already reviewed elsewhere — reuse those findings but re-check realized severity against THIS deployment's config/state.
...
3. **Map & trust boundaries.** Inventory contracts, entry points, **privileged roles** (owner/admin/governor/timelock/upgrader), external calls, oracles, callbacks, and delegatecall/proxy topology. Analyze **compromised-admin and oracle-manipulation paths**, not just external-attacker flows. **TRON custody is account-level, not just Solidity:** for every fund-holding account pull the permission graph via `getaccount` (`owner_permission`/`active_permission`: keys, weights, threshold, operations) and classify custody from THAT — a single active key with weight ≥ threshold is a 1-of-1 EOA even if the source looks multisig-gated (see `tvm-native` in the checklist); feed the real M-of-N into the Centralization class and the report's custody column. **Backdoor / premeditation hunt:** a "verified" badge and a prior audit are NOT trust (both were clean on TronBank Pro, whose runtime carried a hidden trigger). Actively hunt any fund-moving/ownership function gated by an exact-equality on a weird hardcoded literal (a magic sentinel amount/address), and **pivot on the deployer** — enumerate the deployer's OTHER deployments and pre-production txs for a near-identical "rehearsal" contract or a magic-value sentinel tx; premeditated backdoors leave this on-chain footprint even when the source looks clean.
