...
1. **Get source, then build to green.** For an on-chain target, fetch verified source FIRST: `tooling/get-source.sh <address>` pulls every `.sol` + the **exact compiler settings** (`compiler.json`) + ABI from TronScan's `POST /api/solidity/contract/info` (sends `TRON-PRO-API-KEY`), and caches by `code_hash`. **Two distinct trust levels — do not conflate them (they are recorded in `compiler.json`):** (1) `explorer_source_status=2` + `node_runtime_hash_match=MATCH` means only *TronScan says verified* and *the node's runtime is self-consistent* — NOT that this `.sol` produced the deployed code; (2) **source is authoritative only when `recompile_status` is `FULL_MATCH`** (exact `keccak256(deployedBytecode)==on-chain code_hash`) **or `IMMUTABLE_TEMPLATE_MATCH`** (byte-identical except construction-set immutable regions, metadata intact) — both set `source_authoritative=true`. `PARTIAL` (only trailing metadata differs) is strong but not a full byte-proof; `NO_MATCH`/`SKIPPED` means treat the source as unproven and reconcile before relying on it (usually a missing TRON solc fork — run `bootstrap.sh`; or linked libraries, which the recompile does not re-link). `status≠2` → no source exists, drop to the **bytecode-only branch** below. Then detect toolchain (foundry.toml / hardhat.config / tronbox.js / truffle-config / vyper). Install and **pin** the exact compiler (`solc-select`/`svm`, or the TRON `tv_*` solc fork for TVM), install deps, compile. If it won't compile, fix that before reasoning — but **build fixes must never alter in-scope contract logic**; restrict them to toolchain/config/dependency/remapping fixes, and if a source edit is unavoidable, make it in a scratch copy or record it as a `*.patch` under `poc/` and disclose it, keeping the frozen baseline pristine. **Dedup guard (don't re-do the *static* review):** once source + `code_hash` are fetched, run `tooling/review-ledger.sh check <address>`. `REVIEWED-CURRENT` (same `code_hash` + impl + same skill `VERSION`, no explicit re-check) means **the STATIC code review is unchanged — reuse the prior report's findings and skip the code-analysis gates (2, 4, 5, 6, 7).** But **code identity does NOT freeze exploitability: you MUST still re-run the LIVE-STATE gates every time** — gate 3 (custody/permissions via `getaccount`, admin/oracle topology), gate 8 (actually-listed markets/tokens/params, TVL, chain parameters), and re-assess *realized* severity against the current on-chain state (a finding that was latent can become live, and vice-versa). Only fetch/build/static-analysis is cached, never live state. The guard auto-**re-reviews in full** when the `code_hash` changed (redeploy/upgrade), the proxy impl changed, the skill `VERSION` bumped (new knowledge), the address is new, or `RECHECK=1`. `NEW-ADDRESS-SAME-BYTECODE` = identical code reviewed elsewhere — reuse those static findings but do the live-state gates for THIS deployment.
2. **Static analysis (required).** Run `slither` + `aderyn` + `semgrep --config "$AUDIT_HOME/semgrep-smart-contracts/solidity/security"` (the **pinned** Decurity ruleset — use the local clone, NOT the floating registry `p/smart-contracts`, so the run is reproducible) **and `semgrep --config tooling/semgrep-tron/`** (the TRON/TVM-native ruleset — see `tooling/semgrep-tron/README.md`) (+ `mythril` on the highest-value contracts). Capture machine-readable output; triage every hit to true/false/needs-PoC. **A Semgrep match is a POINTER, never a finding** — the TRON rules are LOW/inventory by design; before writing anything up, trace the chain *entrypoint → access control → user data → checks → sink → financial impact* and prove it (gate 7). Record which tools ran, capturing exact versions **and ruleset commits** to `evidence/tool-manifest.txt` (e.g. `forge --version; slither --version; aderyn --version; myth version; semgrep --version; solc --version; git -C "$AUDIT_HOME/semgrep-smart-contracts" rev-parse --short HEAD`) so the run is reproducible — the Decurity ruleset is pinned by commit in `bootstrap.sh` (`DECURITY_REF`), not floating. Also copy each target's `compiler.json` `recompile_status`/`source_authoritative` into the evidence so the source-authority basis is on record. **Legacy compilers:** for `solc <0.6`, some analyzers (e.g. `aderyn`) can't parse the AST — record the tool/version incompatibility as *not-covered* and lean on the analyzers that do work (slither handles 0.5.x); a genuine incompatibility is not the same as "gave up." **Known compiler bugs:** cross-reference the pinned version + the `compiler.json` settings (optimizer/runs/ABIEncoderV2/viaIR/evmVersion) against the [solc known-bugs list](https://docs.soliditylang.org/en/latest/bugs.html) — a vulnerable version is necessary but NEVER sufficient. Confirm all three: (1) exact vulnerable version range, (2) the enabling setting is actually set, (3) a reachable source construct that triggers it. Old TRON 0.4.x/0.5.x contracts are a high-value window (storage-overwrite / experimental-ABIEncoderV2 corruption / dirty-storage families).
