---
name: skill-smartcontracts
description: Use when reviewing, auditing, or security-checking a smart contract or protocol before sign-off — a single Solidity/Vyper contract or a whole repo, on EVM/TVM/TRON. Especially for TRON/TVM targets — a `T...` base58 address, a TronScan/TronGrid contract, TRC-20/TRC-10 tokens, or an unverified/bytecode-only contract. Covers reentrancy, access control, oracle, bridge, proxy/upgrade, DeFi-logic, and TVM-native (TRC-10 id confusion, SUN decimals, staking opcodes) vulnerabilities. Triggers on "audit this contract", "review for vulnerabilities", "security review", "check this TRON/TronScan contract", smart-contract audit.
---
...
1. **Get source, then build to green.** For an on-chain target, do these sub-steps IN ORDER:
   - **1.1 Fetch.** `tooling/get-source.sh <address>` pulls every `.sol` + the **exact compiler settings** (`compiler.json`) + ABI from TronScan's `POST /api/solidity/contract/info` (sends `TRON-PRO-API-KEY`), caches by `code_hash`. `status≠2` → no verified source exists → drop to the **bytecode-only branch** below.
   - **1.2 Attest — two trust levels, do NOT conflate (recorded in `compiler.json`).** (a) `explorer_source_status=2` + `node_runtime_hash_match=MATCH` = only *TronScan says verified* and *the node runtime is self-consistent* — NOT that this `.sol` produced the deployed code. (b) **Source is authoritative only when `recompile_status` is `FULL_MATCH`** (exact `keccak256(deployedBytecode)==code_hash`) **or `IMMUTABLE_TEMPLATE_MATCH`** (byte-identical bar construction-set immutables, metadata intact) — both set `source_authoritative=true`. `PARTIAL` (trailing metadata only) is strong but not a byte-proof; `NO_MATCH`/`SKIPPED` → treat source as unproven and reconcile (usually a missing TRON solc fork → `bootstrap.sh`; or linked libraries, which recompile does not re-link).
   - **1.3 ⛔ DEDUP GUARD — run this BEFORE spending gates 2–9.** `tooling/review-ledger.sh check <address>`. **`REVIEWED-CURRENT`** (same `code_hash`+impl+skill `VERSION`, no explicit re-check) → the STATIC code review is unchanged: **reuse the prior report's findings and SKIP the code-analysis gates (2, 4, 5, 6, 7).** **BUT code identity does NOT freeze exploitability — you MUST still re-run the LIVE-STATE gates every time:** gate 3 (custody/permissions via `getaccount`, admin/oracle topology) + gate 8 (listed markets/tokens/params, TVL, chain parameters), then re-assess *realized* severity (a latent finding can go live, and vice-versa). Only fetch/build/static is cached, never live state. It auto-**re-reviews in full** on: `code_hash` changed (redeploy/upgrade), proxy impl changed, skill `VERSION` bumped, new address, or `RECHECK=1`. `NEW-ADDRESS-SAME-BYTECODE` → reuse static findings, but do the live-state gates for THIS deployment.
   - **1.4 Build to green.** Detect toolchain (foundry.toml / hardhat.config / tronbox.js / truffle-config / vyper), install & **pin** the exact compiler (`solc-select`/`svm`, or the TRON `tv_*` fork for TVM), install deps, compile. If it won't compile, fix that first — but **build fixes must never alter in-scope contract logic** (toolchain/config/dependency/remapping only); an unavoidable source edit goes in a scratch copy or a disclosed `*.patch` under `poc/`, keeping the frozen baseline pristine.
2. **Static analysis (required).** Run `slither` + `aderyn` + `semgrep --config "$AUDIT_HOME/semgrep-smart-contracts/solidity/security"` (the **pinned** Decurity ruleset — use the local clone, NOT the floating registry `p/smart-contracts`, so the run is reproducible) **and `semgrep --config tooling/semgrep-tron/`** (the TRON/TVM-native ruleset — see `tooling/semgrep-tron/README.md`) (+ `mythril` on the highest-value contracts). Capture machine-readable output; triage every hit to true/false/needs-PoC. **A Semgrep match is a POINTER, never a finding** — the TRON rules are LOW/inventory by design; before writing anything up, trace the chain *entrypoint → access control → user data → checks → sink → financial impact* and prove it (gate 7). Record which tools ran, capturing exact versions **and ruleset commits** to `evidence/tool-manifest.txt` (e.g. `forge --version; slither --version; aderyn --version; myth version; semgrep --version; solc --version; git -C "$AUDIT_HOME/semgrep-smart-contracts" rev-parse --short HEAD`) so the run is reproducible — the Decurity ruleset is pinned by commit in `bootstrap.sh` (`DECURITY_REF`), not floating. Also copy each target's `compiler.json` `recompile_status`/`source_authoritative` into the evidence so the source-authority basis is on record. **For a machine-auditable run, emit `evidence/tool-run.json`** — one record per analyzer: `{tool, command, version, ruleset_commit, result_count, status: ran|degraded|not-covered, fallback, promoted_findings}` — so coverage is reconstructable and "we ran X" is falsifiable (e.g. mythril on TVM = `status:degraded`; aderyn on solc<0.6 = `status:not-covered`). **Legacy compilers:** for `solc <0.6`, some analyzers (e.g. `aderyn`) can't parse the AST — record the tool/version incompatibility as *not-covered* and lean on the analyzers that do work (slither handles 0.5.x); a genuine incompatibility is not the same as "gave up." **Known compiler bugs:** cross-reference the pinned version + the `compiler.json` settings (optimizer/runs/ABIEncoderV2/viaIR/evmVersion) against the [solc known-bugs list](https://docs.soliditylang.org/en/latest/bugs.html) — a vulnerable version is necessary but NEVER sufficient. Confirm all three: (1) exact vulnerable version range, (2) the enabling setting is actually set, (3) a reachable source construct that triggers it. Old TRON 0.4.x/0.5.x contracts are a high-value window (storage-overwrite / experimental-ABIEncoderV2 corruption / dirty-storage families).
3. **Map & trust boundaries.** Inventory contracts, entry points, **privileged roles** (owner/admin/governor/timelock/upgrader), external calls, oracles, callbacks, and delegatecall/proxy topology. Analyze **compromised-admin and oracle-manipulation paths**, not just external-attacker flows. **TRON custody is account-level, not just Solidity:** for each privileged **key-controlled** account (owner/admin/governance-executor/treasury/**deployer**) pull the permission graph via `getaccount` (`owner_permission`/`active_permission`: keys, weights, threshold, operations) and classify custody from THAT — a single active key with weight ≥ threshold is a 1-of-1 EOA even if the source looks multisig-gated (see `tvm-native`). A **contract** account has no key (code-governed) — classify its custody as the code's upgrade/withdraw paths, not a permission graph. Feed both into the Centralization class + the report's custody column. **Backdoor / premeditation hunt:** a "verified" badge and a prior audit are NOT trust (both were clean on TronBank Pro, whose runtime carried a hidden trigger). Actively hunt any fund-moving/ownership function gated by an exact-equality on a weird hardcoded literal (a magic sentinel amount/address), and **pivot on the deployer** — enumerate the deployer's OTHER deployments and pre-production txs for a near-identical "rehearsal" contract or a magic-value sentinel tx; premeditated backdoors leave this on-chain footprint even when the source looks clean.
...
6. **Invariants + fuzz.** Write protocol invariants explicitly and test them (Foundry invariant/property tests, Echidna/Medusa, Halmos for symbolic). Don't assert invariants narratively. **crytic-compile / Foundry `evm_version` trap:** when a Foundry config is found up the tree, crytic-compile forwards **Foundry's default `evm_version`** to solc (that default is `osaka` on Foundry ≥1.6, and `prague`/`cancun` on slightly older) — which **every TRON-era compiler (0.4.x–0.7.x, and 0.8.x before that hardfork) rejects**, so `slither`/`echidna`/`medusa` fail to compile (`Invalid option for --evm-version: <hardfork>`) on the old solc most TRON contracts use, and it overrides `--solc-args`. Reliable fix: pin it via **Foundry** — a `foundry.toml` with the right `solc`/`evm_version` (e.g. `solc = "0.6.12"`, `evm_version = "istanbul"`) so the whole crytic/foundry chain honors it. (A bare `slither <repo>` may silently degrade this way — check for the evm-version error.)
7. **Prove every material finding** with an executable PoC (Foundry/fork test that passes on the vulnerable code and fails after the fix). Flag each finding `proven` / `unproven`. The PoC should **exercise the REAL in-scope contract** whenever it compiles; fall back to a clearly-labelled `MODEL` reimplementation only when instantiating the real contract is impractical, and say so. For math-heavy or live-only issues, a **fuzzer/symbolic counterexample** (Echidna/Medusa/Halmos) or **`Empirically-proven-on-chain`** evidence (live tx history / state-reachability reads) also counts as proof. **TVM verification trap (observed, not doc-confirmed):** in one run TronBox rendered *memory* (non-storage) event fields as `undefined`, silently PASSing an assertion against a WRONG emitted value — so do NOT assert PoC correctness off TronBox event fields for memory variables; drive assertions off storage reads or decoded on-chain logs (safe regardless).
...
9. **Mechanical checks over source-level claims** — e.g. prove proxy/implementation storage-layout equivalence with `solc --storage-layout`, not by reading; validate chain/VM assumptions (TVM energy/gas stipend, TRC-20 non-standard returns) against docs/tests. **For TRON/TVM scope specifically:** a stock-`solc` Foundry PoC models a recent EVM hardfork (solc/forge default to Cancun/Prague-era semantics), not the TVM. When a finding turns on a `tvm-native` construct (TRC-10 id, SUN value, a precompile, a staking opcode, CREATE2 derivation), run it on a **real TVM** via **`tooling/tvm-harness/`** — a local java-tron (`tron-quickstart`) sandbox: `up.sh` → deploy+exploit via TronWeb → compare to the EVM-model (`differential.js` is the worked example). A divergence is itself a finding; a match upgrades the PoC from `EVM-model` to **`TVM-proven (re-deploy, not mainnet-state)`**. Otherwise label the PoC `EVM-model` and list the TVM caveats (`.transfer` stipend + Dynamic Energy, energy model, TRC-20 return quirks) that still need on-TVM confirmation. (Neither the local node nor a testnet forks mainnet state, so mainnet-state-dependent behavior still needs read-only on-chain corroboration.)
...
- **[vulnerable-functions.md](vulnerable-functions.md)** — high-risk function patterns by contract type (token, access-control, lending, AMM/DEX, stablecoin, oracle, proxy/upgrade, bridge, liquid-staking, governance, signature, math, general, **erc4626-vault, staking-rewards, perps-derivatives, account-abstraction-4337, eip-7702, intents-solvers, cross-chain-messaging, modular-proxy-diamond, tvm-native**) with the concrete check per function. **access-control, general, and tvm-native apply to every contract — always walk them (tvm-native = the TRON attack surface an EVM checklist misses: TRC-10 id confusion, SUN decimals, precompile divergence, CREATE2 0x41, staking opcodes, delegatecall token-context loss, 21-byte identity).** The AA/7702/intents sections are **EVM-mainly** — each carries a "verify TVM support before assuming" caveat (LayerZero is live on TRON; 4337/7702/Permit2 are not).
- **[vulnerable-chains.md](vulnerable-chains.md)** — multi-step exploit compositions (reentrancy variants, flash-loan→oracle→borrow, share inflation, vault strategy-mark inflation, MasterChef reward drain, perp oracle/liquidation, bridge-mint, cross-chain forgery, governance takeover, signature replay, rounding drain, proxy takeover, AA paymaster drain, 7702 delegate sweep) with the invariant/guard whose presence blocks each.
- **[checklist-ids-and-crosswalk.md](checklist-ids-and-crosswalk.md)** — the stable `VF-<SECTION>-NN` / `VC-<FAMILY>-NN` ID scheme (cite exact items in findings + `coverage.md`) and the crosswalk to OWASP SCSVS (11 control groups) and EEA EthTrust v3.
...
- **Log the review:** once the report is a complete draft, run `tooling/review-ledger.sh record <address> [report]` — it records the target's `code_hash` (+ proxy impl hash), the skill `VERSION`/commit, the date, and the severity counts to `$FINDINGS_DIR/.review-ledger.json`, so the dedup guard (gate 1) will skip a redundant re-audit unless the contract changes or the skill's knowledge `VERSION` bumps. **`VERSION` is the audit-knowledge version** — bump it (minor) whenever the checklists, the `semgrep-tron` rules, or the methodology change, so all prior reviews are correctly flagged stale.
