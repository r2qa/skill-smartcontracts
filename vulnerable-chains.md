...
- **Blocked by (confirm present):** Implementation constructor calls _disableInitializers(); _authorizeUpgrade is owner-gated; no selfdestruct or arbitrary delegatecall in the implementation. *(The selfdestruct-brick half of this chain is neutralized on post-Cancun EVM by EIP-6780 — only same-tx-as-creation deletes code — but still applies on TRON, which has not adopted EIP-6780; the re-init/hijack half applies everywhere.)*
